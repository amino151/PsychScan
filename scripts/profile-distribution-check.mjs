import { profileCatalog } from '../src/config/profiles.js';
import { calculateScores, matchProfile } from '../src/lib/scoring.js';
...
const defaultProfiles = profileCatalog;

const args = process.argv.slice(2);
const sampleArg = args.find((arg) => arg.startsWith('--samples='));
const sampleArgIndex = args.findIndex((arg) => arg === '--samples');
const positionalSample = args.find((arg) => /^\d+$/.test(arg));
const samples = sampleArg
  ? Number(sampleArg.split('=')[1])
  : sampleArgIndex >= 0
    ? Number(args[sampleArgIndex + 1])
    : positionalSample
      ? Number(positionalSample)
    : 10000;

if (!Number.isFinite(samples) || samples <= 0) {
  console.error('Invalid samples count. Use --samples=<positive-number>.');
  process.exit(1);
}

const questions = defaultQuestions.map((question, index) => ({
  ...question,
  id: question.id || `q${index}`,
}));

const histogram = new Map(defaultProfiles.map((profile) => [profile.name, 0]));
const coverage = new Map(defaultProfiles.map((profile) => [profile.name, new Set()]));
...
for (let i = 0; i < samples; i += 1) {
  ...
  const profile = matchProfile(scores, defaultProfiles);

  if (!profile) continue;
  histogram.set(profile.name, (histogram.get(profile.name) || 0) + 1);
  coverage.get(profile.name)?.add(i);
}
...
const idealPct = 100 / defaultProfiles.length;
const maxPct = rows[0]?.pct || 0;
const minPct = rows[rows.length - 1]?.pct || 0;
const spread = maxPct - minPct;

console.log(`Profile distribution check (${samples.toLocaleString()} samples)`);
console.log('='.repeat(68));
rows.forEach((row) => {
  const delta = row.pct - idealPct;
  const sign = delta >= 0 ? '+' : '';
  console.log(
    `${row.name.padEnd(14)} | ${row.count.toString().padStart(5)} | ${row.pct.toFixed(2).padStart(6)}% | ${sign}${delta.toFixed(2)}% vs ideal`
  );
});

console.log('-'.repeat(68));
console.log(`Ideal per profile: ${idealPct.toFixed(2)}%`);
console.log(`Spread (max - min): ${spread.toFixed(2)}%`);
console.log(
  spread <= 12
    ? 'Status: balanced enough for stochastic questionnaire outcomes.'
    : 'Status: noticeable imbalance detected; consider re-tuning trait weights.'
);
